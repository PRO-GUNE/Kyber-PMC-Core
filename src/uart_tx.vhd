...
architecture Behavioral of UART_tx is
  ...
  signal tx_data_index_reset : std_logic                    := '1';
...
begin
  ...
  -- The tx_data_index_counter process is a simple counter from 0 to 7 working on the baud
  ...
  -- The tx_data_index signal is used in UART_tx_FSM to go over the stored_data vector
  -- and send the bits one by one.

  tx_data_index_counter : process (clk)
  begin
    if rising_edge(clk) then
      if (reset = '1') or (tx_data_index_reset = '1') then
        tx_data_index <= 0;
      elsif (baud_rate_clk = '1') then
        tx_data_index <= tx_data_index + 1;
      end if;
    end if;
  end process tx_data_index_counter;
  ...
  begin
    if rising_edge(clk) then
      if (reset = '1') then
        tx_state            <= IDLE;
        tx_data_index_reset <= '1'; -- keep tx_data_index_counter on hold
        start_reset         <= '1'; -- keep tx_start_detector on hold
        tx_data_out         <= '1'; -- keep tx_data_out line set along the standard
        ...
        if (baud_rate_clk = '1') then -- the FSM works on the baud rate frequency
          case tx_state is
            ...
            when START =>
              ...
              tx_data_index_reset <= '0'; -- enable tx_data_index_counter for DATA state
              tx_data_out         <= '0'; -- send '0' as a start bit
              tx_state            <= DATA;

            when DATA =>

              tx_data_out <= stored_data(tx_data_index); -- send one bit per one baud clock cycle 8 times

              if (tx_data_index = 7) then
                tx_data_index_reset <= '1'; -- disable tx_data_index_counter when it has reached 8
                tx_state            <= STOP;
              end if;
